...
def show_plots(root_path: Path) -> None:
    ...
    print(
        "Add with 'aba plots add -d [dir]' and remove with"
        + " 'aba plots remove -d [dir]'"
        + " Scan and check plots with 'aba plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'aba init'")
