...
@click.group(
    help=f"\n  Manage aba blockchain infrastructure ({__version__})\n",
...
@cli.command("version", help="Show aba version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", help="Runs aba daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by aba start <service>
)
@click.pass_context
