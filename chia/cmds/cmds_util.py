...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'aba wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'aba keys generate' or import a key")
